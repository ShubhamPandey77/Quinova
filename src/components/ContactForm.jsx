...
const ContactForm = ({ 
  ...
  return (
    <div 
      className={`bg-white rounded-2xl shadow-2xl w-full ${compact ? 'max-w-lg' : 'max-w-4xl'} h-auto relative transform transition-all duration-300 scale-100 font-sans`}
          ...
          {!compact && (
            <div className="space-y-6">
              <div>
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-3 tracking-tight uppercase italic">
                  Start Your Project
                </h2>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-slate-900 mb-1 text-sm uppercase tracking-wider">Quick Response</h4>
                    <p className="text-xs text-slate-600 font-medium">We'll get back to you within 24 hours</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-slate-900 mb-1 text-sm uppercase tracking-wider">Expert Consultation</h4>
                    <p className="text-xs text-slate-600 font-medium">Free initial consultation with our specialists</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {compact && (
              <div className="mb-4">
                ...
                <h2 className="text-xl font-bold text-slate-900 mb-2 uppercase italic">
                  Start Your Project
                </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {[
                ...
                <div key={id} className="group">
                  <label className="block text-xs font-semibold text-slate-900 mb-1.5 tracking-wide uppercase">
                    {label}
                    {required && <span className="text-slate-400 ml-1">*</span>}
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      disabled={isSubmitting}
                      className="w-full pl-10 pr-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-slate-50/50 focus:outline-none group-hover:bg-white group-hover:border-slate-300 transition-all duration-200 focus:bg-white focus:shadow-sm disabled:opacity-50 disabled:cursor-not-allowed font-medium"
              ...
              <div className="group">
                <label className="block text-xs font-semibold text-slate-900 mb-1.5 tracking-wide uppercase">
                  Project Details <span className="text-slate-400 ml-1">*</span>
                </label>
                <textarea
                  required
                  rows={compact ? 2 : 3}
                  ...
                  disabled={isSubmitting}
                  className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-slate-50/50 resize-none group-hover:bg-white group-hover:border-slate-300 transition-all duration-200 focus:bg-white focus:border-slate-900 focus:shadow-sm disabled:opacity-50 disabled:cursor-not-allowed font-medium"
              ...
              <div className="space-y-3 pt-2">
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold hover:bg-slate-800 transition-all active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-70 disabled:cursor-not-allowed uppercase tracking-widest text-xs"
                >
                  {isSubmitting ? (
                    <>
